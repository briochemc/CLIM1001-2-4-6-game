...
import { verifyLtiLaunch, playerId, signToken, verifyToken } from './auth.js';
...
const VERDICT = new Set(['same', 'partly', 'different']);
...
async function finish(env, session, body) {
  ...
  const now = Math.floor(Date.now() / 1000);
  await env.DB.prepare('UPDATE sessions SET finished = ?, rule_text = ?, confidence = ? WHERE pid = ?')
    .bind(now, ruleText, confidence, session.pid)
    .run();
  return json({ rule_name: RULES[session.rule].name });
}

async function verdict(env, session, body) {
  if (!session.finished) return json({ error: 'Please submit your rule first.' }, 400);
  if (session.verdict) return json({ error: 'You have already answered this.' }, 400);
  const v = String(body.verdict || '');
  if (!VERDICT.has(v)) return json({ error: 'Please choose an answer.' }, 400);

  const keys = [
    'completed',
    `verdict:${v}`,
    `conf:${session.confidence}`,
    `conf:${session.confidence}|verdict:${v}`,
  ];
  const counts = keys.map((k) => [k, 1]);
  counts.push(['attempts', session.n_attempts]);
  counts.push(['no_answers', session.n_no]);
  if (session.n_no > 0) counts.push(['got_a_no', 1]);

  const stmts = counts.map(([k, n]) =>
    env.DB.prepare(
  ...
  stmts.unshift(env.DB.prepare('UPDATE sessions SET verdict = ? WHERE pid = ?').bind(v, session.pid));
...
async function playerState(env, pid) {
  ...
  const state = {
    stage: session.verdict ? 'done' : session.finished ? 'reveal' : 'playing',
...
async function cohortStats(env, ctx, rule) {
  ...
  if (completed < min) return { completed, min_cohort: min, available: false };
  return { completed, min_cohort: min, available: true, counts: t };
}
